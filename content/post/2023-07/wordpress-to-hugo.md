...
今回、ブログの構成を一新し、レンタルサーバーメインの構成から、**AWS**メインの物へと移行することにしました。記事内容を静的サイトジェネレータ（SSG）である**Hugo**に移行してAWS S3でホスティングしています。また、SSL対応やパフォーマンス向上の為CloudFrontをキャッシュサーバーとしています。ドメインもAmazon Route 53へ移管しました。
